...
INV-08  StockChart 입력은 아래 4개를 고정한다.
...
INV-17  2026-09-14 를 구조적 단절일로 고정한다.
        ...
        오버나이트 갭 분포가 변경된다.
        이전 구간의 갭 통계를 이후 구간에 외삽 금지.
        G2 / G10 / G11 은 이전 구간 데이터만으로 최종 통과 판정하지 않는다.
        신규 레짐 데이터 확보 전까지 코어 포지션 50% 제한.
        SDD의 TBD-F11 재추정 완료 전 50% 제한도 함께 적용한다.

INV-18  주문 전송 시 세션을 판정하고, 애프터마켓 구간에서는
        시장가(키움 trde_tp='3') 전송을 코드 레벨에서 차단한다.
        애프터마켓은 지정가·최우선지정가·최유리지정가만 허용된다.
        세션 경계는 하드코딩하지 않고
        CpCodeMgr.GetNewMarketEndTime(code) 로 매 거래일 조회한다.
        공식 도움말에 수능일 16:30 등 예외가 명기되어 있어
        고정 시각은 오작동한다.
